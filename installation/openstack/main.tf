...
resource "openstack_networking_secgroup_rule_v2" "secgroup_https" {
...
resource "openstack_networking_secgroup_rule_v2" "cluster_m2j6bjspw4" {
  description       = "Allow cluster m2j6bjspw4 30744/tcp"
  direction         = "ingress"
  ethertype         = "IPv4"
  protocol          = "tcp"
  port_range_min    = 30744
  port_range_max    = 30744
  remote_ip_prefix  = "0.0.0.0/0"
  security_group_id = openstack_networking_secgroup_v2.securitygroup.id
}
